...
function replain_admin_menu_link () {
	...
	$wp_admin_bar->add_menu( array(
        ...
        'group'  => null,
		'title'  => 'Re:plain',
		'href'   => admin_url( 'options-general.php?page=replain' ),
...
function replain_menu () {
	add_options_page( 'Re:plain', 'Re:plain', 'manage_options', 'replain', 'replain_toplevel_page' );
}
...
function replain_toplevel_page () {
        ...
        if ( wp_verify_nonce( $_POST[$nonce_name], $nonce_action ) ) {
            if ( current_user_can( 'manage_options' ) ) {
        		...
        		if ( isset( $_POST[$code_name] ) ) {
					update_option( $code_name, esc_html($_POST[$code_name] ));
        			$updated = true;
        		}
            } else {
	...
	if ( $code != '' ) {
		echo '<div>
			<h3 class="title">'.__( 'Chat is', 'replain' ).'</h3>

			<select name="'.$enabled_name.'">
	...
	echo '<div>
		<h3 class="title">'.__( 'Enter your Re:plain code', 'replain' ).'</h3>
		<textarea class="code" name="'.$code_name.'" cols="80" rows="8">'.replain_js_code( $code ).'</textarea>
	</div>
	...
	<p>The first ever web-chat service that works directly in your WhatsApp, Facebook Messenger and Telegram.</p>
	</div>';
}
...
function replain_js_code ($code) {
	...
	if (strpos($code, '<script>') === false) {
		$jsCode = '<script>window.replainSettings = { id: \''.$code.'\'};(function(u){var s=document.createElement(\'script\');s.type=\'text/javascript\';s.async=true;s.src=u;var x=document.getElementsByTagName(\'script\')[0];x.parentNode.insertBefore(s,x);})(\'https://widget.replain.cc/dist/client.js\');</script>';
	} else {
		$jsCode = $code;
	}

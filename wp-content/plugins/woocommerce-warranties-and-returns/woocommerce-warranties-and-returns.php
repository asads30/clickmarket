<?php
/*
Plugin Name: Warranties and Returns for WooCommerce |  VestaThemes.com
...
*/
		...
		function war_order( $order ) {
			...
			<h2><?php _e( 'Возврат товара', 'wcwar' ); ?></h2>
			<?php
				if ( !in_array( $order->get_status(), apply_filters( 'wcwar_warranty_order_status', array( 'completed' ), $order ) ) ) {
			?>
			<p class="wcwar_status blue">
				<span class="wcwar_info_icon"><i class="wcwar-info"></i></span> <?php _e( 'Возврат товара станет доступным после того, как вы получите товар!', 'wcwar' ); ?>
			</p>
			<?php
				$curr_complete = get_post_meta( $order->id, '_ordered_date', true );
				$warranty_status_hold[] = '<p class="wcwar_status blue"><span class="wcwar_info_icon"><i class="wcwar-info"></i></span>' . __( 'Отложенный заказ', 'wcwar') . '</p>';
			...
			<table class="shop_table wcwarranty">
				<thead>
					<tr>
						<th><?php _e( 'Товар', 'wcwar' ); ?></th>
						<th><?php _e( 'Гарантия', 'wcwar' ); ?></th>
						<th><?php _e( 'Статус', 'wcwar' ); ?></th>
						<th><?php _e( 'Процесс', 'wcwar' ); ?></th>
					</tr>
				</thead>
				<tbody>
			<?php
				...
				<?php
					...
					if ( $curr_notallowed === false && $curr_status === 'nowar' && $curr_returns == 'yes' && empty( $curr_req ) ) {
						...
						$addon_msg = __( 'Этот товар был продан без гарантии', 'wcwar' );

						if ( self::hlp_valid_return( $curr_complete, $item) === true ) {
							$addon_msg .= __( ',  вы все равно можете вернуть его в магазин, так как его гарантия возврата все еще действительна.', 'wcwar' );
							$warranty_status[] = '<p class="wcwar_status green"><span class="wcwar_info_icon"><i class="wcwar-check"></i></span>' . __( 'Доступен возврат товаров', 'wcwar') . '</p>';
							$curr_link[] = '<a href="' . $curr_request . '" class="wcwar_request_button button">' . __( 'Возврат', 'wcwar') . '</a>';
						}
						else {
							$warranty_status[] = '<p class="wcwar_status grey"><span class="wcwar_info_icon"><i class="wcwar-cross"></i></span>' . __( 'Без гарантии', 'wcwar') . '</p>';
						}
						...
						$addon_msg = __( 'Гарантия на этот товар истекла', 'wcwar' );
						$warranty_status[] = '<p class="wcwar_status grey"><span class="wcwar_info_icon"><i class="wcwar-cross"></i></span>' . __( 'Гарантия истекла', 'wcwar') . '</p>';
						$request_button = false;
					}
...
/*						if ( $requested === false && $curr_multi_req == 'yes' ) {
					...
					if ( isset( $item['wcwar_warranty'] ) ) {
						...
						?>
									...
									_e( 'Этот товар был продан без гарантии', 'wcwar' );
								}
							}
						}
						else {
...
/*						$curr_multi_req = get_option( 'wcwar_enable_multi_requests', 'no' );
						...
						if ( $_GET['item_id'] == '-1' ) {
							...
							?>
								<p class="wcwar_form_info">
									<span class="wcwar_info_icon"><i class="wcwar-info"></i></span>
									<?php echo __( 'There is no warranty for this item.', 'wcwar' ) . ' ' . __( 'Однако вы все равно можете вернуть его в магазин, так как его гарантия возврата все еще действительна.', 'wcwar' ); ?>
								</p>
							<?php
								$only_return = true;
							}

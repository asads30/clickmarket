...
// check notocication activi for all changes
 if ($TelegramNotify2->getValuefromconfig('notify_woocomerce_order_change') && (!$options)) {
	 ...
	 } elseif($old_status === 'dpd-send'){
		 $old_status = 'готов к отправке dpd';
	 } elseif($old_status === 'wc-shipped'){
		 $old_status = 'Заказ собран';
	 } elseif($old_status === 'wc-custom-order-status'){
		 $old_status = 'Заказ передан курьеру';
	 } else {
	 ...
	 } elseif($new_status === 'dpd-send'){
		 $new_status = 'готов к отправке dpd';
	 } elseif($new_status === 'wc-shipped'){
		 $new_status = 'заказ собран';
	 } elseif($new_status === 'wc-custom-order-status'){
		 $new_status = 'заказ передан курьеру';
	 } else {
